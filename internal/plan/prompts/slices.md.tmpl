...
- Produce `{{.TasksDir}}/TASKS.md` (overview) and one `{{.TasksDir}}/TASK<N>.md` per task
- Do NOT write code
...
Done when `{{.TasksDir}}/TASKS.md` and all `{{.TasksDir}}/TASK<N>.md` files are written, every PRD capability is mapped to a task, and the sequencing rules are followed.
