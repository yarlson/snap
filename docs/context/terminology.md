...
**Phase 2 (Document generation)** — Autonomous phase of planning where Claude generates planning documents: PRD.md (requirements), TECHNOLOGY.md (architecture), DESIGN.md (specifications), and TASKS.md (task summary). Document generation uses a 6-step pipeline: PRD generated sequentially, TECHNOLOGY and DESIGN generated concurrently via errgroup, then a 4-step task-splitting chain (create task list, assess against anti-patterns, refine via merge/split, generate TASKS.md summary) executed sequentially with conversation continuity. All Phase 2 prompts are prepended with shared engineering principles to guide consistent decision-making across all generated documents.
