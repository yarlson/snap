...
- [`cli/plan.md`](cli/plan.md) — Plan command, two-phase planning pipeline, tap.Text styled interactive input (TTY) and buffered scanner input (pipes), autonomous document generation, --from flag, session resolution, plan resumption, provider integration
