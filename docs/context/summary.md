...
- **Plan command**: `snap plan [session]` with two-phase pipeline: Phase 1 interactive requirements gathering (raw-mode input on TTY, buffered input on pipes), Phase 2 autonomous document generation (PRD, TECHNOLOGY, DESIGN, TASK files); conflict guard detects existing planning artifacts (prompts user in TTY, errors in non-TTY); supports `--from <file>` to skip Phase 1; auto-creates "default" session if none exist; session auto-detection for single-session projects; Ctrl+C in Phase 1 aborts planning gracefully
