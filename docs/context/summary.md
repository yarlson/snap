...
- **Plan command**: `snap plan [session]` with two-phase pipeline: Phase 1 interactive requirements gathering (tap.Textarea multiline input on TTY, buffered input on pipes), Phase 2 autonomous document generation (PRD, TECHNOLOGY, DESIGN, TASK files) with parallel execution of TECHNOLOGY and DESIGN documents; guided by engineering principles (KISS, DRY, SOLID, YAGNI); conflict guard detects existing planning artifacts and offers choices in TTY (clean up and re-plan, or create new session), errors in non-TTY; supports `--from <file>` to skip Phase 1; auto-creates "default" session if none exist; session auto-detection for single-session projects; Ctrl+C in Phase 1 aborts planning gracefully
