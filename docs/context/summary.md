...
- **Planning workflow**: `snap plan [session]` generates structured planning documents (PRD, TECHNOLOGY, DESIGN, TASK files) through two-phase pipeline (interactive requirements gathering with tap.Text styled input for TTY or buffered input for pipes, then autonomous document generation); includes conflict guard to detect and handle existing planning artifacts; auto-creates "default" session on fresh projects with no sessions
...
- **Plan command**: `snap plan [session]` with two-phase pipeline: Phase 1 interactive requirements gathering (tap.Text styled input on TTY, buffered input on pipes), Phase 2 autonomous document generation (PRD, TECHNOLOGY, DESIGN, TASK files); conflict guard detects existing planning artifacts and offers choices in TTY (clean up and re-plan, or create new session), errors in non-TTY; supports `--from <file>` to skip Phase 1; auto-creates "default" session if none exist; session auto-detection for single-session projects; Ctrl+C in Phase 1 aborts planning gracefully
...
- **Interactive terminal components**: github.com/yarlson/tap (Text input with validation, Select prompts with arrow-key navigation)
